...
benchmark: benchmark-setup
	...
	@RESULTS_DIR=benchmark/results go test -bench=. -benchmem -benchtime=10s -run=^$ -v ./benchmark/scenarios/...
	@echo "Results saved to benchmark/results/"

benchmark-short-lived: benchmark-setup
	@RESULTS_DIR=benchmark/results go test -bench=BenchmarkShortLived -benchmem -benchtime=10s -run=^$ -v ./benchmark/scenarios/...

benchmark-high-concurrency: benchmark-setup
	@RESULTS_DIR=benchmark/results go test -bench=BenchmarkHighConcurrency -benchmem -benchtime=10s -run=^$ -v ./benchmark/scenarios/...

benchmark-mixed-sizes: benchmark-setup
	@RESULTS_DIR=benchmark/results go test -bench=BenchmarkMixedSizes -benchmem -benchtime=10s -run=^$ -v ./benchmark/scenarios/...
